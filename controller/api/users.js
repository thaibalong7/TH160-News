...
exports.register = async (req, res) => {
    try {
        ...
        if (await helper.validateEmail(req.body.email)) {
            const check_user = await subscribers.findOne({
                where: {
                    email: email
                }
            });
            if (!check_user) {
                ...
                let check_id_user = await subscribers.findAll({ where: { id: id_user } })

                while (!check_id_user) {
                    id_user = await helper.generateIDUser(4);
                    check_id_user = await subscribers.findAll({ where: { id: id_user } })
                }
                const new_user = {
                    id: id_user,
                ...
                subscribers.create(new_user).then(_subcriber => {
